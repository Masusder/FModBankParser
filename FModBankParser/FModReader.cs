...
public class FModReader
{
    ...
    private void ParseNodes(BinaryReader Ar, long start, long end)
    {
        ...
        while (Ar.BaseStream.Position + 8 <= end)
        {
            ...
            {
                if (nodeId != ERIFFID.CHUNKID_LIST)
                {
    ...
    public bool TryGetSoundSampleFromSoundTable(string key, out FmodSample? sample)
    {
        ...
        {
            Debug.WriteLine("Sound table or sound bank data is missing, cannot retrieve sound info");
            return false;
        }
        ...
        {
            Debug.WriteLine($"Sound with key '{key}' (hash {hash:X}) not found in sound table");
            return false;
        }
    ...
    public FModGuid GetBankGuid() => BankInfo.BaseGuid;
